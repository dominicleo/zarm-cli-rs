...
type GetContextInnerType<T extends Context<any>> = T extends Context<infer R>
  ? R
  : never;
type nameType = keyof Omit<GetContextInnerType<typeof LocaleContext>, "locale">;

const ConfigReceiverWrapper = (
  name?: nameType,
  defaultConfig?: typeof defaultConfigData
) => {
  return function InnerWrapper<T extends {}>(
    WrappedComponent: ComponentType<T>
  ) {
    const ConfigReceiver = <P extends { forwardedRef: React.Ref<T> }>(
      props: P
    ) => {
      return (
        <ConfigContext.Consumer>
          {({ locale, safeIphoneX }) => {
            ...
            const componentLocale =
              globalLocale[name || (WrappedComponent.name as nameType)];
