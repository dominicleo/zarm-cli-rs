...
const customArrowOffsetFn = (data: PopperJS.Data) => {
  ...
  const arrow =
    data.instance.options.modifiers &&
    (data.instance.options.modifiers!.arrow!.element as Element);
  const {
    offsets: { reference },
  } = data;
  ...
  const offsetSize = parseFloat(
    getComputedStyle(data.instance.popper, null).paddingLeft!
  );
  const hashMap = {
    start:
      side === "top" || side === "left"
...
class Popper extends React.Component<PopperProps, PopperStates> {
  static update() {
    popperInstances.forEach((popperInstance) =>
  ...
  componentDidUpdate(prevProps: PopperProps) {
    ...
    if (
      visible &&
      (prevProps.visible !== visible || prevProps.direction !== direction)
    ) {
      this.handleOpen();
    }
