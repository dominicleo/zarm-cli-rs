...
const getTouches = (touches: TouchList): Array<Point> =>
  Array.from(touches).map(
    (totuch: Touch): Point => ({ x: totuch.pageX, y: totuch.pageY })
  );
...
const PinchZoom = React.forwardRef<unknown, PinchZoomProps>((props, ref) => {
  ...
  const handleZoom = (event, newScale) => {
    const touchCenter = getTouchCenter(
      getTouchesOffset(getTouches(event.touches))
    );
      ...
      switch (interaction) {
        case "zoom":
          if (startTouches.length === 2 && event.touches.length === 2) {
            handleZoom(
              event,
              calculateScale(startTouches, getTouches(event.touches))
            );
          }
          break;
        case "drag":
